							...
							if (acl[i].visitable == 1) {
								...
								switch (onlyid) {
									case 0:
										tempobjs = {
											name: '附近换电',
											url: '/map/map?name=换电&text=全部换电&type=0',
											val: 1
										}
									...
									case 3:
										tempobjs = {
											name: '单个挪车',
											url: '/map/map?text=全部车站&type=3.1&name=挪车',
											val: 2
										}
										this.qxmenudata.push(tempobjs)
										break
		...
		methods: {
			...
			gonewpage(index, item, indexs) {
				...
				uni.navigateTo({
					...
					url: `/map/map?text=${item.name}&type=10&name=车辆监控${spliturl}`,
					success: res => {},
					fail: () => {},
					complete: () => {}
				});
						...
						if (type == 1) {
							...
							this.boxdata = [
								...
								},
								{
									...
									url: '/map/map?name=维修&text=全部故障车辆&type=1.1'
								},
								{
									name: '挪车数',
