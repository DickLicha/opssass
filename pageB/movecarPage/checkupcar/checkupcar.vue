<template>
	<view class='wrap'>
		<view class='view-common'>
			<item-cell v-if="type!=101" :itemdata="swapdata" type='4' :border='borders' @itemclick='gocarcenter'></item-cell>
			...
			<view>
				<map class='base-map-view' :show-location='true' v-show='showmap' @markertap='markclick' :scale="scale" :latitude="latitude"
				 :longitude="longitude" :markers="covers" @controltap='mapcentionloc' :circles="circles" :controls='maploc' :style="{height:mapheihts}">
				</map>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			if (e.type == 99) {
				...
				wx.startLocationUpdate({
					success:(res)=>{
						console.log('kaiqijianting',res)
						wx.onLocationChange((data)=>{
							console.log('位置变化',data)
							this.getmovingbike([data.longitude, data.latitude])
						})
					},
					fail:(err)=>{
						console.log('jiantingshibai',err)
					}
				})
				// if (this.timmer == null) {
		...
		methods: {
			...
			getmovingbike(weizhi) {
				var jinweidu=[this.longitude, this.latitude]
				if(!!weizhi){
					console.log(555,weizhi)
					jinweidu=weizhi
				}
				var options = {
				...
				this.$httpReq(options).then((res) => {
					...
					if (res.status == 0) {
						...
						// console.log(2222, this.covers, this.circles)
					} else {
						uni.showToast({
							title: res.message ? res.message : '获取车辆信息失败',
							mask: false,
