<template>
	<view>
		...
		<view class="picker-content" :class="{'pickerShow':pickerShow}">
			...
			<view class='five-class' v-if="mode=='five'">
				<text>请输入要关联对象</text>
				<input class="search-view" type="text" v-model="searchText" value="" placeholder="搜索" @input="searchInput" />		
				<scroll-view class="search-result-view" scroll-y="true">
					<view class="search-result-content-view" v-for="(item, index) in searchResult" :key="index" v-show="searchResult.length != 0"
					 @click="seachResultClick(item)">
						{{item.name}}
					</view>
				</scroll-view>
			</view>
