	...
	export default {
		...
		methods: {
			...
			go(item) {
				if(this.type==0){
					uni.navigateTo({
						url: `/pageA/endingview/endingview?type=${this.type}&&bikeid=${item.bike_id}&&orderid=${item.id}`,
						success: res => {},
						fail: () => {},
						complete: () => {}
					});
					...
					this.$httpReq(options).then((res) => {
						...
						if (res.status == 0) {
				            ...
				            uni.navigateTo({
				            	url: `/pageB/movecarPage/checkupcar/checkupcar?type=99&&bikeid=${item.bike_id}&&orderid=${item.id}`,
				            	success: res => {},
				            	fail: () => {},
				            	complete: () => {}
				            });
