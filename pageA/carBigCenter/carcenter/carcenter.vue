		...
		data() {
			return {
				...
				carcenterdata3: [{
						name: '订单',
						val: '',
						url: '/pageA/carBigCenter/carorder/carorder'
					},
					{
						name: '车辆操作记录',
						val: '',
						url: '/pageB/switchloockinfo/switchloockinfo'
					},
					{
						name: '换电记录',
						val: '',
						url: '/pageA/carBigCenter/batteriesrecord/batteriesrecord'
					},
					{
						name: '维修记录',
						val: '',
						url: '/pageA/carBigCenter/carorbit/carorbit'
					},
					{
						name: '车辆轨迹',
						val: '',
						url: '/pageA/map/map?type=0.1&&name=用户骑行轨迹'
					},
					{
						name: '最后一次扫码时手机位置',
						val: '',
						url: '/pageA/map/map?type=0.2&&name=最后扫码位置'
					},
					{
						name: '车辆当前位置',
						val: '',
						url: '/pageA/map/map?type=0.3&&name=车辆当前位置'
					},
					{
						name: 'ECU',
						val: '',
						url: '/pageA/carBigCenter/ecuview/ecuview'
					},
					{
						name: '电池',
						val: '',
						url: '/pageA/carBigCenter/batteryview/batteryview'
					},
					{
						name: '最近上报日志',
						val: '',
		...
		methods: {
			...
			fuzhi(){
				try{
					...
					// 车锁状态
					let is_wheel_locked = ''
					if (this.bikeinfo.is_defend_on == 0) {
						is_wheel_locked = '开'
					} else if (this.bikeinfo.is_defend_on == 1) {
						is_wheel_locked = '关'
					}
					this.carcenterdata2[4].val = is_wheel_locked
			...
			gocarcenter(item) {
				...
				switch (e.item.val) {
					...
					case '3':
						uni.navigateTo({
							url: '/pageB/movecarPage/checkupcar/checkupcar',
							success: res => {},
							fail: () => {},
							complete: () => {}
						});
