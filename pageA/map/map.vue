...
case '10':
		...
		methods: {
					...
					if (pointtype == 'stop') {
						if (this.endmove) {
							...
							if (this.clicksuccess == false) {
								uni.navigateTo({
									url: `/pageB/movecarPage/stopdetilview/stopdetilview?name=${pointname}&&bickcount=${bickcount}&&allcount=${allkcount}&&id=${e.markerId}`,
									success: res => {
			...
			dowhat() {
				switch (this.type) {
					case '0':
						this.urls = '/pageB/swapbattery/swapbattery?type=0'
						break;
					case '1.3':
						this.urls = '/pageB/repairPage/repaircar/repaircar'
						break;
					case '1.1':
						this.urls = '/pageB/repairPage/putstorage/putstorage'
						break;
					case '2':
						this.urls = '/pageA/maintainPage/normalmaintain/normalmaintain'
						break;
					case '3.1':
						this.urls = '/pageB/movecarPage/checkupcar/checkupcar'
						break;
					case '10':
						this.urls = '/pageB/swapbattery/swapbattery?type=0'
			...
			goingview() {
				uni.navigateTo({
					url: `/pageA/ingview/ingview?type=${this.type}`,
					success: res => {},
					fail: () => {},
					complete: () => {}
				});
