				...
				this.$httpReq(options).then((res) => {
					...
					if (res.status == 0) {
						...
						for (var i in respondata.user_count_daily) {
							...
							// 订单数
							user_order_growth.push(respondata.urorder_count_daily[i])
							...
							if(this.chartlimits){
								// 订单金额
								user_order_amount_growth.push(respondata.urorder_paid_amount_daily[i] / 100)
						...
						this.avercarratio=(allbikeeffic/Object.keys(respondata. user_count_daily).length).toFixed(2)
						this.avermoney=(allmoney/Object.keys(respondata. user_count_daily).length).toFixed(2)
						// 倒序
